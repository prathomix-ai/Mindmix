...
export default function ErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    ...
    console.error("[MindMix Runtime Boundary Caught Error]:", error);
  }, [error]);
      ...
      <div className="max-w-md w-full p-8 rounded-2xl bg-zinc-950/80 border border-zinc-800 backdrop-blur-xl shadow-2xl text-center space-y-6 relative z-10">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-zinc-400 leading-relaxed font-mono">
            A runtime exception was safely intercepted by the MindMix error boundary. Your canvas session data remains preserved in local storage.
          </p>
        </div>
